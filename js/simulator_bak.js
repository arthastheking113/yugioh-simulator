...
var hand = [];
var board = [];
var currentDeck = joinMainExtraDeck(playtest.main, playtest.extra);
...
$('#playerdeck, #view').click(function () {
    openDeck(currentDeck);
})
...
function openDeck(deck, extra = false) {
    ...
    for (i in (extra ? extradecklist : decklist)) {
        if (extra)
            $('#deckmenu').append(`<img style="margin: 1px" src="asset/card/${extradecklist[i].name}.jpeg" width="60px" id="${extradecklist[i].id}"/>`)
        else
            $('#deckmenu').append(`<img style="margin: 1px" src="asset/card/${decklist[i].name}.jpeg" width="60px" id="${decklist[i].id}"/>`)
    }
...
function addCardToHandDiv(i, extra) {
    if (extra == 0)
    // get from https://yugiohprices.com/api/card_image/${decklist[i].name}
        $('#hand').append(`<div id="cardId${decklist[i].id}" class="testcard-slot-row"><div class="hand cardMain${decklist[i].id}"><img id="${decklist[i].id}" src="asset/card/${decklist[i].name}.jpeg" /></div>`)
    else if (extra == 1)
        $('#hand').append(`<div id="cardId${extradecklist[i].id}" class="testcard-slot-row"><div class="hand cardEx${extradecklist[i].id}"><img id="${extradecklist[i].id}" src="asset/card/${extradecklist[i].name}.jpeg" /></div>`)
    ...
    $(nameDom).draggable({
        ...
        stack: '.hand',
        stop: function () {
            console.log( 'draggable stopped', $(this), extra, snappedToDeck )
...
function snappedEvent(cardDOM, extra, event) {
    ...
    $.each(draggable.snapElements, function (index, element) {
        if (element.snapping) {
            ...
            refreshHand();
            if (!snappedTo.some(e => e.id != 'gy-slot')) {
